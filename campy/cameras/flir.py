...
def GetSerialNumber(device):
    node_device_serial_number = PySpin.CStringPtr(
        device.GetTLDeviceNodeMap().GetNode("DeviceSerialNumber")
    )
...
def GetModelName(camera):
    node_device_model_name = PySpin.CStringPtr(
        camera.GetTLDeviceNodeMap().GetNode("DeviceModelName")
    )
...
def StartGrabbing(camera):
    try:
        ...
        print(
            "Exception in cameras/flir.py function StartGrabbing(camera): {}".foramt(e)
        )
        return False
    ...
    while True:
        try:
            try:
                ...
                logging.error(
                    "Caught exception at cameras/flir.py CloseCamera: {}".format(e)
                )
                raise
                break
...
def ConfigureCustomImageSettings(camera, cam_params):
    ...
    try:
        ...
        logging.error(
            "Caught error at cameras/flir.py ConfigureCustomImageSettings: {}".format(e)
        )
...
def ConfigureBuffer(camera, cam_params):
    ...
    try:
        ...
        print(
            "BufferMode has been set to {}".format(handling_mode_entry.GetDisplayName())
        )
        ...
        logging.error(
            "Caught exeption at cameras/flir.py ConfigureBuffer: {}".format(e)
        )
...
def ConfigureChunkData(camera, cam_params):
    ...
    try:
        ...
        for chunk_selector_entry in entries:
            if PySpin.IsAvailable(chunk_selector_entry) and PySpin.IsReadable(
                ...
                if cam_params["cameraDebug"]:
                    ...
                    elif not chunk_enable.GetValue() and PySpin.IsWritable(
                        chunk_enable
                    ):
        ...
        logging.error(
            "Caught error at cameras/flir.py at ConfigureChunkData: {}".format(e)
        )
...
def ConfigureFrameHeight(camera, cam_params):
    ...
    try:
        ...
        logging.error(
            "Caught exception at cameras/flir.py ConfigureFrameHeight: {}".format(e)
        )
...
def ConfigureFrameWidth(camera, cam_params):
    ...
    try:
        ...
        logging.error(
            "Caught exception at cameras/flir.py ConfigureFrameWidth: {}".format(e)
        )
...
def ConfigureGain(camera, cam_params):
    ...
    try:
        ...
        if not PySpin.IsAvailable(node_gainauto_mode) or not PySpin.IsWritable(
            node_gainauto_mode
        ):
...
def ConfigureGamma(camera, cam_params):
    ...
    try:
        if cam_params["disableGamma"]:
            ...
            if not PySpin.IsAvailable(node_gamma_enable_bool) or not PySpin.IsWritable(
                node_gamma_enable_bool
            ):
...
def ConfigurePixelFormat(camera, cam_params):
    ...
    try:
        ...
        if pixelFormat == "bayer_rggb8" or pixelFormat == "bayer_bggr8":
            ...
            eval(
                "camera.PixelFormat.SetValue(PySpin.PixelFormat_{}".format(pixelFormat)
            )
...
def ConfigureTrigger(camera, cam_params):
    ...
    try:
        ...
        if (
            ...
            eval(
                "camera.TriggerSource.SetValue(PySpin.TriggerSource_{})".format(
                    cameraTrigger
                )
            )
