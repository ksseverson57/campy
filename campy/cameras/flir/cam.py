...
def ConfigureTrigger(camera, cameraTrigger: str):
	"""
...
def ConfigureExposure(camera, exposureTime: int):
	"""
...
def ConfigureGain(camera, gain: float):
	"""
...
def ConfigurePixelFormat(camera, pixelFormat: str):
	"""
	...
	try:
		...
		if pixelFormat == "bayer_rggb8" or pixelFormat == "bayer_bggr8":
			...
			try:
				...
				print('Could not set ISP enable.')
		...
		print("Error setting pixel format: %s" % ex)
		return False

	return result


def DisableGamma(camera, disableGamma: bool):
...
def ConfigureBuffer(camera, bufferMode="OldestFirst", bufferSize=100):
	result = True
...
def EnableChunkDataPayloads(camera):
	"""
...
def ConfigureCustomImageSettings(camera, cam_params):
	...
	try:
		...
		try:
			...
			logging.error("Caught exception at cam.py GetMax: {}".format(e))
...
def OpenCamera(cam_params):
	try:
		...
		logging.error('Caught exception at cam.py OpenCamera: {}'.format(e))
...
def LoadSettings(cam_params, camera):
	...
	# Configure trigger
	cam_params["trigConfig"] = ConfigureTrigger(camera, cam_params["cameraTrigger"])
	...
	# Configure exposure, gain, gamma, buffer and chunkdata mode (e.g. timestamp/frame info)
	ConfigurePixelFormat(camera, cam_params["pixelFormatInput"]) 
	ConfigureExposure(camera, cam_params["cameraExposureTimeInUs"])
	ConfigureGain(camera, cam_params["cameraGain"])
	ConfigureBuffer(camera, cam_params["bufferMode"], cam_params["bufferSize"])
	EnableChunkDataPayloads(camera)
	DisableGamma(camera, cam_params["disableGamma"])
...
def DisplayImage(cam_params, dispQueue, grabResult):
	try:
		...
		img = GetImageArray(grabResult, cam_params)
		...
		logging.error('Caught exception at cam.py DisplayImage: {}.'.format(e))
