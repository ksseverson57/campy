...
def GrabFrames(
    ...
    # Get stride for this camera
    frameStride = int(cam_params["frameStride"])
    if frameStride < 1:
        frameStride = 1
    ...
    while(True):
        if stopGrabQueue or frameNumber >= grabdata["numImagesToGrab"]:
            ...
            if grabbing:
                ...
                if cam_params["startArduino"] and triggerQueue:
                    cam_params["triggers"] = triggerQueue.popleft()
        ...
        if grabbing:
            try:
                ...
                # ---- STRIDE GATE ----
                if (frameNumber % frameStride) != 0:
                    # IMPORTANT: release/drop this frame to avoid buffer buildup
                    cam.ReleaseFrame(grabResult)  # or cam.ReleaseFrame/grabResult.Release depending on API
                    frameNumber += 1
                    continue

                # Compute time elapsed and count fps
                timeElapsed = timeStamp - grabdata["timeStart"]
                frameCount = frameNumber // frameStride
                CountFPS(grabdata, frameCount, timeElapsed)
...
def SaveMetadata(cam_params, grabdata):
    ...
    try:
        ...
        frame_count = grabdata['frameNumber'] // cam_params["frameStride"]
        ...
        meta['totalFrames'] = int(math.floor(grabdata['frameNumber'] / cam_params["frameStride"]))
