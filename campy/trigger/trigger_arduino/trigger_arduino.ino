// arduino triggering sync
// inter-frame interval precision is currently ~ 10 us
// synchronicity precision is ~ 10 ns

int DIG_out_pins[] = {2, 4, 7, 8, 12, 13}; // 3, 5, 6, 9, 10, 11 = PWM
int n_sync = 1;
volatile unsigned long frame_rate = 0;
...
void loop( void ) {
  ...
  // If message sent, stop triggers
    if (Serial.available() == 1) {
      frame_rate = Serial.parseInt();
      if (frame_rate == 0) { // Timeout (no input) sends '0'
      }
      else if (frame_rate == -1) { // encode -1 as stop
        frame_period = 0xFFFFFFFF;
        pulse_duration = 0xFFFFFFFF;
      }
      else {
